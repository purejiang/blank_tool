...
#
# This module is the single public entry point for environment helpers
# (paths, binary resolution, dotenv/server-config loading).  It used to live
# at ``app.utils.env``, which has been removed — ``app.env`` is now the only
# home for the env API.  Pure path helpers (``ROOT`` / ``resolve_path`` /
# ``get_runtime_dir``) live in :mod:`app.utils.paths`; this module re-uses
...
def get_cache_dir() -> str:
    ...
    (resolved relative to the cli root). Creates the directory on disk if
    it does not exist.
    """
...
def _resolve_binary_from_registry(env_name: str) -> str:
    ...
    try:
        ...
        resolved = get_env_registry().resolve(env_name)
