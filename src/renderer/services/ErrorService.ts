...
interface ErrorContext {
    ...
    retryCount?: number;
    serviceName?: string;
    serviceManager?: { restartService: (serviceName: string) => Promise<unknown> };
    configService?: { resetToDefault: () => Promise<unknown> };
    filename?: string;
    lineno?: number;
    colno?: number;
    [key: string]: unknown;
...
class ErrorService {
    private notificationService: { error?: (title: string, message: string) => void } | null;
    private errorHandlers: Map<string, Array<(errorInfo: ErrorInfo) => Promise<unknown> | unknown>>;
    private errorLog: ErrorInfo[];
    private maxLogSize: number;
    private listeners: Set<(event: string, data: unknown) => void>;
    private recoveryStrategies: Map<string, (error: unknown, context: ErrorContext) => Promise<RecoveryResult>>;
    private errorCategories: Record<string, string>;

    constructor(notificationService?: { error?: (title: string, message: string) => void } | null) {
    ...
    registerDefaultRecoveryStrategies() {
        ...
        this.registerRecoveryStrategy(this.errorCategories.NETWORK, async (_error, context) => {
            ...
            // 重试网络请求
            if (context.retryFunction && (context.retryCount || 0) < 3) {
                try {
                    await new Promise(resolve => setTimeout(resolve, 2000 * ((context.retryCount || 0) + 1)));
                    ...
                    return {
                        success: false,
                        message: `重试失败: ${retryErrorMessage}`
                    };
                }
            }
        ...
        this.registerRecoveryStrategy(this.errorCategories.SERVICE, async (_error, context) => {
            ...
            if (context.serviceName && context.serviceManager) {
                try {
                    ...
                    return {
                        success: false,
                        message: `服务重启失败: ${restartErrorMessage}`
                    };
                }
            }
        ...
        this.registerRecoveryStrategy(this.errorCategories.CONFIG, async (_error, context) => {
            ...
            if (context.configService) {
                try {
                    ...
                    return {
                        success: false,
                        message: `配置重置失败: ${resetErrorMessage}`
                    };
                }
            }
     ...
     */
    async handleError(error: unknown, context: ErrorContext = {}) {
     ...
     * @param {Object} errorInfo - 错误信息
     */
    async executeErrorHandlers(errorInfo: ErrorInfo) {
     ...
     */
    getLogLevel(severity: ErrorSeverity) {
        switch (severity) {
     ...
     */
    registerErrorHandler(category: string, handler: (errorInfo: ErrorInfo) => Promise<unknown> | unknown) {
     ...
     */
    registerRecoveryStrategy(category: string, strategy: (error: unknown, context: ErrorContext) => Promise<RecoveryResult>) {
     ...
     */
    addListener(listener: (event: string, data: unknown) => void) {
     ...
     * @param {Function} listener - 监听器函数
     */
    removeListener(listener: (event: string, data: unknown) => void) {
     ...
     */
    getErrorLog(filters: ErrorLogFilters = {}) {
