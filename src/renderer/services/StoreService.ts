...
interface AppConfigStoreLike {
    config: AnyRecord
    defaultConfig?: AnyRecord
    saveConfig?: (config: AnyRecord) => Promise<unknown>
    resetConfig?: () => Promise<unknown>
    getConfig?: (key: string, defaultValue?: unknown) => Promise<unknown>
    setConfig?: (key: string, value: unknown) => Promise<unknown>
    update?: (config: AnyRecord) => Promise<unknown>
    reset?: () => Promise<unknown>
}

class ConfigStoreService {
    private appConfigStore: AppConfigStoreLike | null
    private userConfigStore: AnyRecord | null
    private deviceStore: AnyRecord | null
   ...
   async initialize() {
    ...
    async resetSettings() {
        try {
            const store = await this.ensureAppConfigStore()
            if (typeof store.resetConfig === 'function') {
    ...
    async setSetting(key, value) {
        try {
            ...
            if (typeof store.setConfig === 'function') {
                ...
                this.setNestedValue(nextConfig, key, value)
                await store.update(nextConfig)
            }
            return { success: true }
        } catch (error) {
        ...
        try {
            ...
            if (defaultValue !== undefined) {
                if (typeof store.setConfig === 'function') {
                    ...
                    const nextConfig = { ...(store.config || {}) }
                    this.setNestedValue(nextConfig, key, defaultValue)
                    await store.update(nextConfig)
                }
            }
