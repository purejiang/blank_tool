...
import { useAppConfigStore, useUserConfigStore, useDeviceStore } from '../stores'
...
class ConfigStoreService {
   ...
   async initialize() {
        try {
            ...
            console.warn('无法初始化 app config store、user config store 或 device store，将在首次使用时重试')
        }
    }
        ...
        try {
            ...
            return {
                user: {
                    ...
                    language: config.app?.language || 'zh-CN'
                }
            }
        } catch (error) {
