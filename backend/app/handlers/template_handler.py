...
def handle_execute(params, stream_handler):
    ...
    # The engine forwards ``context.stream_handler`` straight through to the
    # builtin tools, so it must stay a plain callable receiving event dicts.
    # The WorkflowStreamHandler is a separate emit-API wrapper around that
    # callback: create it when a task id + callback are available so the
    # terminal workflow event is emitted with the workflow_id metadata.
