...
- **零外部依赖**：仅使用内置原子工具（`file.*`、`dir.*`、`text.grep`、`flow.*` 等），无需任何外部二进制或系统工具。
...
- 当前包含 5 个工作流：download-install、decompile、recompile、sign、aab-install。
...
```bash
# 通用示例（无需任何准备）
python backend/cli.py run examples/workflows/generic/dir-report.json -i target_dir=.
python backend/cli.py run examples/workflows/generic/text-scanner.json -i target_dir=./src -i pattern="import"

# Android 示例（需先导入描述符 + 自备二进制）
python backend/cli.py run examples/workflows/android/decompile.json -i apk_path=app.apk
```
