...
class _StubDescriptorTool:
    ...
    The execute method is injected at construction so each test can
    return the exact dict it needs without forking a real process.
    The tool_path must be non-empty for validation in DescriptorTool,
    so we pass a dummy value that won't exist on disk.
    """
